...
%DIF DEL version1/sap_optimum.tex   Thu Sep  9 10:42:33 2021
%DIF ADD sap_optimum.tex            Wed Nov 24 09:13:32 2021
%%                                      %
...
%DIF 52c52
%DIF < %\RequirePackage{hyperref}
%DIF -------
\RequirePackage{hyperref} %DIF > 
%DIF -------
...
The primary outcome is a binary indicator of history of IgE-mediated food allergy at the age of 12 months confirmed, where necessary, with an oral food challenge before 18 months of age. 
Secondary outcomes include: 
(1) history of parent-reported clinician-diagnosed new onset of atopic dermatitis by 6 or 12 months of age with a positive skin prick test to any allergen before 12 months of age; 
...
\DIFadd{Operating characteristics of trial decision rules were evaluated by trial simulation.
The selected rules for success and futility approximately maintain type I error of 0.05 and achieved power 0.85 for a reduction in the primary outcome from 10\% in the control group to 7\% in the intervention group.
}

\DIFaddend \parttitle{Discussion} %if any
...
\DIFaddbegin \DIFadd{The notions of mechanistic plausibility that support these results are based on the Th$_1$/Th$_{17}$ immune-modulating properties of some components of }\textit{\DIFadd{Bordetella pertussis}} \DIFadd{within wP formulations, which could promote the development of oral tolerance in children predisposed to maintain Th$_2$- biased responses into later infancy \mbox{%DIFAUXCMD
\cite{perez2020}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFaddend We hypothesise that a single dose of whole-cell vaccine in early infancy is protective against IgE-mediated food allergy in early childhood.
...
\DIFaddbegin \DIFadd{Stage 1 was designed to obtain detailed solicited adverse event data following each primary pertussis vaccine dose, and post-priming immune response data for the first 150 infants only. 
Stage 2 was designed as a simpler protocol with less intensive follow-up and fewer visits scheduled.
}

\DIFaddend During stage one, 150 participants were enrolled at Perth Children's Hospital (PCH), Western Australia between March 2018 and January 2020.
...
\subsection*{\DIFdelbegin \DIFdel{Elimination Criteria During The Study}\DIFdelend \DIFaddbegin \DIFadd{Exclusion from per-protocol population}\DIFaddend }
...
\begin{enumerate}[resume]
	...
	IgE levels $\geq$ \DIFdelbegin \DIFdel{0.35 }\DIFdelend \DIFaddbegin \DIFadd{0.10 }\DIFaddend kU/L are considered positive.
	\DIFaddbegin \DIFadd{The total IgE low-range assay can detect concentrations between 0.1 to 100 kU/L.
...
\begin{enumerate}[resume]
	...
	vaccine antigen-specific IgG concentrations/titres will be measured at approximately ages 6, 7, 18, and 19 months using a multiplex fluorescent bead assay \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{van2008development}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{van2008development, McAlister2021}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , for the following during stage 1 only (150 participants).
	\begin{enumerate}
	...
	\end{enumerate}
	The seropositive thresholds\DIFaddbegin \label{lab:sero-thresholds} \DIFaddend for pertussis antigens are:
	\begin{itemize}
	...
	\DIFaddbegin \DIFadd{For pneumococcal IgG responses the lower limit of quantification ranges from 0.3 ng/mL to 1.9 ng/mL \mbox{%DIFAUXCMD
\cite{menon2012children}}\hspace{0pt}%DIFAUXCMD
.
	For the other antigens-specific IgG, the lower limits of quantification range from 0.01 mIU/mL (DT) to 0.43 mIU/mL (FHA) \mbox{%DIFAUXCMD
\cite{McAlister2021}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend \end{enumerate}
...
\DIFdel{Parental satisfaction with vaccination: to }\DIFdelend \DIFaddbegin 

 \subsection*{\DIFadd{Vaccine Satisfaction}}

\DIFadd{To }\DIFaddend assess parental dissatisfaction with vaccination in each group after each scheduled vaccine encounter, the following will be collected:
 \begin{enumerate}
...
\DIFdelbegin \DIFdel{The allocation sequence }\DIFdelend \DIFaddbegin \DIFadd{An electronic copy of the randomisation list is distributed by the study statistician to the unblinded pharmacist for printing and concealment.
The randomisation list }\DIFaddend is concealed from all blinded research staff in a non-transparent envelope until completion of the study.
An unblinded pharmacist or research nurse obtains the next contiguous allocation \DIFaddbegin \DIFadd{from the concealed list (i.e. the lowest available randomisation number) }\DIFaddend and prepares the study or comparator vaccine into a clear 1 ml ready administer syringe, labelled with the study participant's number and their identifiers.
...
\DIFaddbegin \DIFadd{This effect size was selected on the basis of consultation with clinical stakeholders.  
}\DIFaddend 
...
\begin{table}[!ht]
	...
	\begin{tabular}{lp{8cm}}
		...
		Stage 1 assays (PP) & All participants in the per-protocol set who were enrolled under stage 1 protocol at PCH/TKI site and who met none of the stage 1 \DIFdelbeginFL \DIFdelFL{elimination }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{per-protocol exclusion }\DIFaddendFL criteria and who did not receive any contraindicated vaccines or medications for stage 1.\\
...
\DIFaddbegin \DIFadd{Each model will be inferred separately for the ITT and PP analysis sets.
}\DIFaddend Interest lies in the measure of effect of assignment to the study vaccine on the primary endpoint relative to the comparator vaccine, denoted by \(\delta\).
...
\DIFaddbegin \DIFadd{Due to the timing of the primary outcome, the number of interim analyses is conditional on enrolment rates.
Under the assumed accrual rate (Table \ref{tab:scenarios}), approximately 10 interim analyses are expected.
}


\DIFaddend The process for interim decisions is as follows.
Let $\mathbb P(\delta < 0|D_k)$ denote the posterior probability of superiority under the specified model \DIFdelbegin \DIFdel{given dataup to interim $k=1,2,...,K$ where complete data from $n_k$ participants are available}\DIFdelend \DIFaddbegin \DIFadd{conditional on data, $D_k$, at interim $k=1,...,K$}\DIFaddend .
...
Let $y_i^k$ denote the number of participants in arm $i$ out of $n_i^k$ who \DIFdelbegin \DIFdel{had a response }\DIFdelend \DIFaddbegin \DIFadd{met the primary outcome criteria, }\DIFaddend and $D_k$ denote \DIFdelbegin \DIFdel{all }\DIFdelend the available data.
...
$$
where $q=0.95$, $\overline{c}_k=0.95$, and $\underline{c}_k=0.05$ for all $k$ which were selected based on simulations \DIFaddbegin \DIFadd{(see }\nameref{sec:oc}\DIFadd{)}\DIFaddend .
...
Results outside the limits of detection will be reported as \DIFdelbegin \DIFdel{above or below the detection limit }\DIFdelend \DIFaddbegin \DIFadd{below or above the limit of quantification}\DIFaddend . 
...
\begin{itemize}
	...
	\item the proportion of concentrations \DIFdelbegin \DIFdel{above/below detection limits }\DIFdelend \DIFaddbegin \DIFadd{below or above limits of quantification
	}\DIFaddend \item the proportion of concentrations exceeding the positive response threshold \DIFaddbegin \DIFadd{(see page \pageref{lab:sero-thresholds})
	}\DIFaddend \item Kaplan-Meier plots of concentrations
\end{itemize}
...
An LKJ(1) prior is specified on the correlation matrix, and \DIFdelbegin \DIFdel{truncated Cauchy with mean }\DIFdelend \DIFaddbegin \DIFadd{half-Cauchy with location }\DIFaddend 0 and \DIFdelbegin \DIFdel{standard deviation }\DIFdelend \DIFaddbegin \DIFadd{scale }\DIFaddend 10 on the marginal standard deviation parameters.
...
Inferences for relative proportions exceeding positive response threshold by study group will assume logistic regression models with a fixed term for treatment group, visit, and their interaction\DIFaddbegin \DIFadd{, and random intercepts for participants}\DIFaddend .
...
\begin{itemize}
	\item the median\DIFaddbegin \DIFadd{, IQR, }\DIFaddend and range of concentrations
	\item the proportion of missing concentrations
	\item the proportion of concentrations \DIFdelbegin \DIFdel{above/below detection limits
	}\DIFdelend \DIFaddbegin \DIFadd{below the limit of quantification
	}\DIFaddend \item the proportion of concentrations exceeding the seroprotective/seropositivity thresholds \DIFaddbegin \DIFadd{(see page \pageref{lab:sero-thresholds})
	}\DIFaddend \item the proportion of participants with a 4-fold or greater increase in their concentration from 18 to 19 months 
...
Non-inferiority of the wP vaccine strategy compared to the standard aP vaccine strategy with respect to PT will be assessed against a non-inferiority margin of 2/3 on the GMR \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{who}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
The number and proportions of infants in each study group with each type of solicited AE and their intensity grading will be summarised for each day following vaccination (day 0 to 6) for each vaccine occasion (2, 4, 6, and 18 months in stage 1; 2 and 18 months in stage \DIFdelbegin \DIFdel{2.
}\DIFdelend \DIFaddbegin \DIFadd{2).
}\DIFaddend The number and proportion of infants in each study group with each type of solicited AE, and at least one type of solicited AE for each vaccine occasion will be reported.

\DIFaddbegin \subsection*{\DIFadd{Vaccine Satisfaction}}

\DIFaddend The responses for vaccination satisfaction of primary caregivers for infants in each study group on the 5-point Likert scale on day 7 after each vaccine occasion will be summarised by counts and proportions.
...
\section*{Operating Characteristics}\DIFaddbegin \label{sec:oc}
\DIFaddend 
...
\DIFaddend The final thresholds were selected informally on the basis of their type I error, type II error, and expected sample size.
\DIFaddbegin \DIFadd{For example, based on the final row of Figure \ref{fig:superiority_16}, tresholds consistent with type I error of approximately 0.05 were identified.
...
Results are presented here only for the 16 per week accrual scenario; the other scenarios are available in a supplemental file (Additional file 1)\DIFaddbegin \DIFadd{.
The R functions and scripts used to simulate the trials and summarise the results are available at }\url{https://github.com/jatotterdell/OPTIMUMsims}\DIFaddend .
